                        ...
                        <div class="inner-form">
                            <div class="input-field first-wrap mb-0">
                                <div class="input-select">
                                    <select class="choices" data-trigger="" name="choices-single-default"
                                    data-images-ris="{{ route('ris',0) }}"
                                    data-videos-ris="{{ route('video.ris',0) }}"
                                    data-vectors-ris="{{ route('vectors.ris',0) }}"
                                    data-images-ris-modal-title="{{ __('Search similar :type',['type'=>__('global.images')]) }}"
                                    data-videos-ris-modal-title="{{ __('Search similar :type',['type'=>__('global.videos')]) }}"
                                    data-vectors-ris-modal-title="{{ __('Search similar :type',['type'=>__('global.vectors')]) }}"
                                    data-images-ris-description="{{ __('Only support JPG and PNG images under :size MB',['size'=>20]) }}"
                                    data-videos-ris-description="{{ __('Only support JPG and PNG images under :size MB',['size'=>20]) }}"
{{--                                    data-videos-ris-description="{{ __('Only support JPG,PNG,MP4,AVI images or videos under :size MB',['size'=>20]) }}"--}}
                                    data-vectors-ris-description="{{ __('Only support JPG and PNG images under :size MB',['size'=>20]) }}"
                                    >
                                        <option value="images"
                                                @if(strpos(url()->current(),url(app()->getLocale().'/photos')) === 0) selected @endif>
                                            <i class="far fa-camera-alt"></i> {{trans('views.Images')}}
                                        </option>
                                        <option value="videos"
                                                @if(strpos(url()->current(),url(app()->getLocale().'/videos')) === 0) selected @endif>
                                            <i class="far fa-video"></i> {{trans('views.Videos')}}
                                        </option>
                                        <option value="vectors"
                                                @if(strpos(url()->current(),url(app()->getLocale().'/vectors')) === 0) selected @endif>
                                            <i class="far fa-tilde fa-lg"></i> {{trans('global.vectors')}}
                                        </option>
                                    </select>
                                </div>
                            </div>
                            <div class="input-field second-wrap d-flex align-items-center">
                                <input
                                    onkeyup="validationnavbar()"
                                    onkeydown="validationnavbar()"
                                    onclick="validationnavbar()"
                                    onchange="validationnavbar()"
                                    onkeypress="validationnavbar()"
                                    maxlength="50"
                                    id="search"
                                    type="text"
                                ...
                                <div id="search-alert" style="background-color: #FFFFFF;display: none">
                                    {{__('misc.sentence_is_to_long')}}
                                </div>
                            </div>
        ...
        <div class="modal-dialog modal-sm modal-dialog-centered" role="document">
            <div class="modal-content p-3">
                <div class="modal-header align-items-center">
                    <h2 class="my-0">{{ __('Search similar :type',['type'=>__('global.images')]) }}</h2>
...
<div class="featured-keyword">
    <ul>
        ...
        @if($search_word_images)
            @foreach($search_word_images as $key=>$search_wordItem)
                <li class="featured-keyword__images">
                    <a href="{{route('search',$search_wordItem->key_word)}}">{{$search_wordItem->key_word}}</a> <?php echo $key == 9 ? "" : "," ?>
                </li>
            @endforeach
        @endif
        @if($search_word_videos)
            @foreach($search_word_videos as $key=>$search_wordItem)
                <li class="featured-keyword__videos d-none">
                    <a href="{{route('video.search',$search_wordItem->key_word)}}">{{$search_wordItem->key_word}}</a> <?php echo $key == 9 ? "" : "," ?>
                </li>
            @endforeach
        @endif


        @if($search_word_vectors)
            @foreach($search_word_vectors as $key=>$search_wordItem)
                <li class="featured-keyword__vectors d-none">
                    <a href="{{route('vectors.search',$search_wordItem->key_word)}}">{{$search_wordItem->key_word}}</a> <?php echo $key == 9 ? "" : "," ?>
                </li>
            @endforeach
