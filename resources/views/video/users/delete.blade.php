...
<div class="container margin-bottom-40">
		...
		<div class="col-md-12">
			<div class="wrap-center center-block">
				...
				<form action="{{ route('account.delete') }}" method="post" name="form" class="margin-top-20">
					<input type="hidden" name="_token" value="{{ csrf_token() }}">
					<div class="btn-block text-center">
						<button type="submit" id="buttonSubmit" class="btn btn-lg btn-main custom-rounded">{{ trans('misc.yes_confirm') }}</button>
             		  <a href="{{ route('account') }}" class="btn btn-default btn-lg custom-rounded">{{ trans('misc.cancel_confirm') }}</a>
					</div>
				</form>
