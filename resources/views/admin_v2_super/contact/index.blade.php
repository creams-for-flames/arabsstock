...
@section('toolbar')
    <a href="{{ route('admin.super.contact.export') }}"
       class="btn btn-label-brand "> {{__('views.Export')}} </a>
@endsection
@section('content')
    <style type="text/css">
        .btn-bold{
        ...
        var KTUserListDatatable = function () {
                    ...
                    columns: [{
...

                        {
                            ...
                            template: function (row) {
                                ...
                                var state = '';
                                var edit_url = '{{$edit_url}}'.replace('/0', '/' + row.id) + '?return_page=' + datatable.getCurrentPage()

                                var output = '<div class="kt-user-card-v2">\
              <div class="kt-user-card-v2__pic">\
                <div class="kt-badge kt-badge--xl kt-badge--">' + row.name.substring(0, 1) + '</div>\
              </div>\
              <div class="kt-user-card-v2__details">\
                <a href="' + edit_url + '" class="kt-user-card-v2__name">' + row.name + '</a>\
                <span class="kt-user-card-v2__desc">' + row.email + '</span>\
              </div>\
            </div>';

                                return output;
                                // return '<img src="{{asset('avatar').'/'}}'+row.avatar+'" width="20" height="20" class="img-circle mr-2" />'+row.username+'';
                            }
                        },
                        {
                            field: 'images',
                            ...
                            template: function (row, index, datatable) {
                                ...
                                if (row.images && row.images.length > 0)
                                    image = `<img src="${row.images[0].image}" width="50" height="50" />`;
                                return image;
                            },
                            sortable: false,
                            textAlign: 'center',
                        ...
                        },
                        {
                            ...
                            template: function (row) {
                                var edit_url = '{{$edit_url}}'.replace('/0', '/' + row.id) + '?return_page=' + datatable.getCurrentPage()
                                var delete_url = '{{$destroy_url}}'.replace('/0', '/' + row.id) + '?return_page=' + datatable.getCurrentPage()
                                return '\
              <div class="dropdown">\
            ...
            ';
                ...
                datatable.on('kt-datatable--on-init', function () {
                    ...
                    datatable.gotoPage({{ request('return_page') }})
                    @endif
                });
            }
                        ...
                        swal.fire({
                            ...
                            if (result.value) {
                                ...
                                post(action_url, {
                                    ...
                                    return_page: datatable.getCurrentPage(),
