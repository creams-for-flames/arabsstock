...
@section('title'){{ __('global.myVectors').' - ' }}@endsection
@section('profile_content')
    <div class="container my-downloads">
        <h3 class="pb-3">{{__('global.myVectors')}}</h3>
        <ul class="nav mb-3" id="pills-tab" role="tablist">
            @if(count($types))
                @foreach ($types as $type)
                    <li class="nav-item" role="presentation">
                        <button
                            class="mb-0 nav-link  @if ($loop->first) active @endif"
                            id="{{$type}}-tab"
                            data-toggle="pill"
                            onclick="ChangeDownloadType('{!! $type !!}')"
                            data-target="#{{$type}}"
                            type="button"
                            role="tab"
                            aria-controls="{{$type}}"
...
@push('javascript_navbar')
    <script>
        ...
        var my_downloads_url = "{{route('me.vectors')}}";
        $(function () {
