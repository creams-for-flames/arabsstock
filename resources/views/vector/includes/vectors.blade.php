...
@foreach( $vectors as $vector )
  @if( Auth::check() )
<?php
    $collection = new \App\Models\VectorCollection();
    $collections = $collection->where( 'user_id', \Auth::user()->id )->orderBy( 'id', 'asc' )->get(); ?> @endif
         <div class="item card-photo"  data-w="{{($vector->width_thumbnail)}}px" data-h="{{($vector->height_thumbnail)}}px">
                <div class="hover border-file">
                    ...
                    <img class="w-100" srcset="{{ cdn($vector->thumbnail) }}" src="{{ cdn($vector->thumbnail) }}"  width="{{($vector->width_thumbnail??300)}}" height="{{($vector->height_thumbnail??300)}}" alt="{{$vector->title}}">
                    ...
                    <div class="card-photo-content">
                        ...
                        <div class="icon">
                            <div class="d-flex flex-row-reverse">
                                ...
                                <div class="icon_save" onclick="showModal('{{$vector->id}}','{{class_basename($vector)}}','{{ cdn($vector->thumbnail) }}','{{$vector->title_en}}')">
