<?php
	...
	$downloadsCount = App\Models\Image::join('downloads', function($join) use($userID) {
...
@if( Auth::check() && Auth::user()->id == $user->id )
	<!-- *********** COVER ************* -->
      <form class="pull-left myicon-right position-relative" style="z-index: 100;" action="{{route('cover.upload')}}" method="POST" id="formCover" accept-charset="UTF-8" enctype="multipart/form-data">
...
<div class="col-md-12">
	...
	<div class="center-block text-center profile-user-over">
		...
		<a href="{{ url($user->username) }}">
        		<img src="{{ asset('avatar').'/'.$user->avatar }}" width="125" height="125" class="img-circle border-avatar-profile avatarUser" />
        		</a>
...
@if( Auth::check() && Auth::user()->id == $user->id )
	<!-- *********** AVATAR ************* -->
      <form action="{{route('avatar.upload')}}" method="POST" id="formAvatar" accept-charset="UTF-8" enctype="multipart/form-data">
    		<input type="hidden" name="_token" value="{{ csrf_token() }}">
           ...
           @if( $user->website != ''
			             ...
			             @if( $user->website != '' )
    						...
    						@if( $user->twitter != '' )
    							...
    							@if( $user->google != '' )
    							<a target="_blank" href="{{ e($user->google) }}" title="Google +"  class="urls-bio icons-bio" data-toggle="tooltip" data-placement="top">
	   ...
	   <ul class="nav nav-pills inlineCounterProfile">
        	 ...
        	 @if( Auth::check() && Auth::user()->id == $user->id )
        	  <li>
        	  	...
        	  	<a href="{{route('photos.pending')}}" class="text-muted link-nav-user">{{trans('misc.photos_pending')}}</a>
        	  </li><!-- End Li -->
        	  @endif
        	  ...
        	  <li>
    			...
    			<li>
    				...
    				<a href="{{route('user.followers',$user->username)}}" class="text-muted link-nav-user">{{trans('users.followers')}}</a>
    			</li><!-- End Li -->

    			<li>
    				<small class="btn-block sm-btn-size text-left counter-sm">{{ App\Helper::formatNumber($user->following()->count()) }}</small>
    				<a href="{{route('user.following',$user->username)}}" class="text-muted link-nav-user">{{trans('users.following')}}</a>
    			</li><!-- End Li -->

    			<li>
    				<small class="btn-block sm-btn-size text-left counter-sm">{{ App\Helper::formatNumber($user->collections()->count()) }}</small>
    				<a href="{{route('user.collections',$user->username)}}" class="text-muted link-nav-user">{{trans('misc.collections')}}</a>
    			</li><!-- End Li -->
    	</ul>
...
<div class="modal fade" id="reportUser" tabindex="-1" role="dialog" aria-hidden="true">
     		<div class="modal-dialog modal-sm">
     			<div class="modal-content">
				    ...
				    <!-- form start -->
			    <form method="POST" action="{{ route('report.user') }}" enctype="multipart/form-data" id="formReport">
			    	<input type="hidden" name="_token" value="{{ csrf_token() }}">
...
//<<---- PAGINATION AJAX
        $(document).on('click','.pagination a', function(e){
        	...
        	'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
					...
					url: '{{ route("landPage") }}/ajax/user/images?id={{$user->id}}&page=' + page
