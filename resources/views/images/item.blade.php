<div class="item card-photo" data-w="{{ $image->width_thumbnail }}px" data-h="{{ $image->height_thumbnail }}px">
    <div class="hover border-file h-100">
        <a href="{{ $image->post_link }}">
            <img class="w-100 h-100" srcset="{{ cdn($image->thumbnail) }}" src="{{ cdn($image->thumbnail) }}" alt="{{ $image->title }}" width="{{(int)($image->width_thumbnail)}}" height="{{(int)($image->height_thumbnail)}}">
        ...
        <div class="card-photo-content">
            ...
            <div class="icon">
                <div class="d-flex flex-row-reverse">
                    <div class="icon_save">
                        <span>
                            <i data-id="{{ $image->id }}" data-like="{{ trans('misc.like') }}" data-unlike="{{ trans('misc.unlike') }}" data-type="{{class_basename($image)}}" class="fal fa-heart @if ($image->is_like) active @endif likeButton"></i>
                        </span>
                        <span>{{ __('misc.like') }}</span>
                    </div>
                    <div class="icon_save" onclick="showModal('{{$image->id}}','{{class_basename($image)}}','{{ cdn($image->thumbnail) }}','{{ $image->title }}')">
