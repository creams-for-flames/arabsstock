@if (isset($data) and $data->count())
    ...
    <div class="flex-images btn-block margin-bottom-40 dataResult">
        @foreach ($data as $image)
            <div class="item card-photo" data-w="{{($image->width_thumbnail)}}px"
                 data-h="{{($image->height_thumbnail)}}px">
                <div class="hover border-file">
                    ...
                    @if($image->downloader_id!=auth()->id())
                        @php($downloader=\App\Models\User::find($image->downloader_id))
                        @if($downloader)
                            <span data-toggle="tooltip"
                                  data-placement="top"
                                  title="<span>{{ $downloader->name }}</span> </br><span>{{ $image->pivot_date }}</span>"
                            >
                    ...
                    <div class="card-photo-content">
                        ...
                        @if($download_type==='exclusive')
                            <div class="icon">
                                ...
                                : {{ format_date('d M Y',$image->reserved_until) }}
                                    </span>
                                    @if($image->reserved_until < now())
...
@else
    <section class="border rounded p-4">
        ...
        <small>
            <a href="{{ route('photos.home') }}"> {{ __('You can download from here') }} </a>
        </small>
    </section>
@endif
<script>
