...
<div class="col-md-12">
	...
	<div class="center-block text-center profile-user-over">
		...
		<a href="{{ route('account.profile',$user->username) }}">
        	 ...
        	 @if( Auth::check() && Auth::user()->id == $user->id )
        	  ...
        	  <li>
    			...
    			<li>
    				...
    				<a href="{{route('profile.followers',$user->username)}}" class="text-muted link-nav-user">{{trans('users.followers')}}</a>
    			</li><!-- End Li -->

    			<li>
    				<small class="btn-block sm-btn-size text-left counter-sm">{{ App\Helper::formatNumber($user->following()->count()) }}</small>
    				<a href="{{route('profile.following',$user->username)}}" class="text-muted link-nav-user">{{trans('users.following')}}</a>
    			</li><!-- End Li -->

    			<li>
    				<small class="btn-block sm-btn-size text-left counter-sm">{{ App\Helper::formatNumber($user->collections()->count()) }}</small>
    				<a href="{{route('video.user.collections',$user->username)}}" class="text-muted link-nav-user">{{trans('misc.collections')}}</a>
    			</li><!-- End Li -->
    	</ul>
...
@if( Auth::check() && $user->id != Auth::user()->id && $user->paypal_account != '' || Auth::guest()  && $user->paypal_account != '' )
 <form id="form_pp" name="_xclick" action="https://www.paypal.com/cgi-bin/webscr" method="post"  style="display:none">
    ...
    <input type="hidden" name="return" value="{{route('account')}}">
    <input type="hidden" name="cancel_return"   value="{{route('account')}}">
