...
?>
				...
				<div class="media li-group noty-media">
					<div class="pull-left">
						<a href="{{ route('account.profile',$key->username) }}"> <img width="40" height="40" class="img-circle myicon-righ media-objectt" alt="User" src="{{ asset('avatar').'/'.$key->avatar }}"> </a>
					</div>
					<div class="media-body">
						<div class="pull-right small">
							<span class="timestamp timeAgo" data="{{ date('c', strtotime( $key->created_at )) }}"></span>
						</div>
						<h5 class="media-heading"><a href="{{ route('account.profile',$key->username) }}"> <strong>@if( $key->name != '') {{ e( $key->name ) }} @else {{ e( $key->username )  }} @endif</strong> </a></h5>
