...
<div class="kt-container  kt-container--fluid  kt-grid__item kt-grid__item--fluid">
    ...
    <div class="kt-portlet kt-portlet--mobile">
        <div class="kt-portlet__body kt-portlet__body--fit">
            <!--begin: Datatable -->
            <div class="kt-datatable" id="kt_video_payments_items_all_list_datatable"></div>
            <!--end: Datatable -->
        </div>
    </div>
    <!--end::Portlet-->
...
var KTUserListDatatable = function() {
  ...
  var init = function() {
    ...
    datatable = $('#kt_video_payments_items_all_list_datatable').KTDatatable({
...
saveState: {
      ...
      },
      {
        ...
        template: function(row, index, datatable) {
          if(row.order.country)
          return row.order.country.name_ar +" | "+row.order.city.name_ar;
          return '---';
        },
      },
      {
      ...
      },
      {
        ...
        template: function(row, index, datatable) {
          ...
          if(row.video && row.video.thumbnail )
            return '  <img src="{{url('')}}' + row.video.thumbnail + '" width="50"/>';
          return '';
        },
      },

      {
        ...
        template: function(row, index, datatable) {
          if(row.video && row.video.parent && row.video.parent.title)
            var show_url = '{{$show_url}}'.replace('0', row.video.parent.id)
          if(row.video && row.video.parent && row.video.parent.title)
            return '<a target="_blank" href="'+show_url+'">'+row.video.parent.title+'</a>';
          return '';
         },
      },
      {
        field: 'resolution',
        title: '{{__('global.stock.fields.resolution')}}',
        template: function(row, index, datatable) {
          if(row.video && row.video.parent )
            return row.video.type;
          return '';
         },
      },
