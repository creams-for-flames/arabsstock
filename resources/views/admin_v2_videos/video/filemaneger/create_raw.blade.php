                    ...
                    <div class="col-xl-12">
                        <div class="kt-portlet">
                            <div class="kt-portlet__head">
                                <div class="row mt-4 mb-5 border">
                                    <div class="col-12">
                                        <h3>  رفع ملف رو  للفيديو # {{$video->id}} </h3>
                                    </div>
                                    <div class="col-lg-2 mt-4">
                                        <a href="{{ $video->post_link??'#' }}">
                                            <img class="w-100 m-1" srcset="{{ cdn($video->thumbnail) }}"
                                    ...
                                    <div class="col-lg-9">
                                        <div class="mt-4 pb-4">
                                            <p class="text-muted published mb-0">
                                                <span> {!! __("admin.video") .' '.__('admin.'.$video->original_type) !!} </span>
                                            </p>
                            ...
                            <div class="kt-portlet__body">
                                <form id="fileupload" action="https://jquery-file-upload.appspot.com/" method="POST"
                                    ...
                                    <div class="row fileupload-buttonbar">
                                        <div class="col-lg-10">
                                            ...
                                            <span class="btn btn-success fileinput-button">
                                                ...
                                                <span>استعراض...</span>
                                                <input type="file" name="file" accept=".mov" />
                                            </span>
                                    ...
                                    <input type="hidden" name="original_type" value="raw">
...
@push('scripts')
            ...
            if (window.File && window.FileReader && window.FileList && window.Blob) {
                if ($(this).val()) { //check empty input filed
                    ...
                    if (!rFilter.test(oFile.type)) {
                        $('#filePhoto').val('');
                        $('.popout').addClass('popout-error').html("{{ trans('misc.formats_available') }}").fadeIn(
                            500).delay(5000).fadeOut();
                        return false;
                    }
                    ...
                    if (fsize > allowed_file_size) {
                        ...
                        $('.popout').addClass('popout-error').html(
                            "{{ trans('misc.max_size') . ': ' . App\Helper::formatBytes($settings->file_size_allowed * 1024) }}"
                            ).fadeIn(500).delay(5000).fadeOut();
                        return false;
                    }
                    ...
                    oFReader.onload = function(e) {
                        ...
                        image.onload = function() {
                            ...
                            if (image.width < {{ $dimensions[0] }}) {
                                $('#filePhoto').val('');
                                $('.popout').addClass('popout-error').html(
                                    "{{ trans('misc.width_min', ['data' => $dimensions[0]]) }}").fadeIn(
                                    500).delay(5000).fadeOut();
                                return false;
                            }
                            ...
                            if (image.height < {{ $dimensions[1] }}) {
                                $('#filePhoto').val('');
                                $('.popout').addClass('popout-error').html(
                                        "{{ trans('misc.height_min', ['data' => $dimensions[1]]) }}")
                                    .fadeIn(500).delay(5000).fadeOut();
                                return false;
                            }
    ...
    <script>
        var fi = $('#fileupload'); //file input
            ...
            timeout: 300000000,
            url: '{{ env('MEDIA_URL') . '/api/admin/videos/raw' }}',
            autoUpload: false,
            headers: {
                'Authorization': 'Bearer {{ $accessToken }}',
                'Language': '{{ config('app.locale') }}',
            },
            acceptFileTypes: /(\.|\/)(mov)$/i,
