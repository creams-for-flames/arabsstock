...
<div class="kt-container  kt-container--fluid  kt-grid__item kt-grid__item--fluid">
    ...
    <div class="kt-portlet kt-portlet--mobile">
        <div class="kt-portlet__body kt-portlet__body--fit">
                            ...
                            <div class="row align-items-center">
                                <div class="col kt-margin-b-20-tablet-and-mobile">
                                    <div class="form-group">
                                        ...
                                        <input type="text" name="date_from" class="form-control" id="kt_datepicker_1"
                                ...
                                <div class="col-md-3 kt-margin-b-20-tablet-and-mobile">
                                    <div class="form-group">
                                        ...
                                        >
                                            <option value=""></option>
                                            @foreach(\App\Models\User::where('role','admin_video')->get() as $r)
            ...
            <!--begin: Datatable -->
            <div class="kt-datatable" id="kt_video_list_datatable"></div>
            <!--end: Datatable -->
        </div>
    </div>
    <!--end::Portlet-->
...
@push('scripts')
<script>
const lang = "{{config('app.locale')??'ar'}}";
...
var KTUserListDatatable = function() {
  ...
  var init = function() {
    ...
    datatable = $('#kt_video_list_datatable').KTDatatable({
...
saveState: {
      ...
      },
      {
       ...
       template:function (r){
           return ('<a href="{{ route('video.show',':slug') }}" target="_blank">' + r.title + '</a>').replace(':slug', r.slug)
       }
      }, {
        field: 'thumbnail',
        ...
        sortable: false,
        template: function (row) {
          return '  <img src="{{cdn('')}}/' + row.thumbnail + '" width="200" class="img-thumbnail"/>';
        },
      }, {
        field: 'user_id',
...

        {
          ...
          template: function(row) {
            var statuses = {
              ...
              '0': {
                'title': '{{__('admin.has_not_noise')}}',
                'class': ' btn-label-success'
              },
            };
            return '<span class="btn btn-bold btn-sm btn-font-sm ' + statuses[row.has_noise].class + '">' + statuses[row.has_noise].title + '</span>';
          }
        },
        {
        field: "Actions",
        ...
        template: function(row) {
          ...
          var price_url = '{{$price_url}}'.replace('0',row.id);
          var delete_url = '{{$destroy_url}}'.replace('0',row.id);
          var create_raw = '{{$create_raw}}'.replace('0',row.id);
          return `
              <div class="dropdown">
                ...
                <div class="dropdown-menu dropdown-menu-right">
                  <ul class="kt-nav">
                    @if (auth()->user()->role === 'admin_video')
                    ${row.raw == null && row.contributor_video_id == 0?`                    <li class="kt-nav__item">
                      <a  href="${create_raw}" target="_blank"  " class="kt-nav__link">
                        <i class="kt-nav__link-icon flaticon2-add"></i>
                        <span class="kt-nav__link-text">{{__('views.add_raw')}}</span>
                      </a>
                    </li>`:''}
                    ...
                    <li class="kt-nav__item">
                      <a href="${price_url}" class="kt-nav__link">
                        <i class="kt-nav__link-icon flaticon-price-tag"></i>
                        <span class="kt-nav__link-text">{{__('views.Price')}}</span>
                      </a>
                    </li>
                    <li class="kt-nav__item">
