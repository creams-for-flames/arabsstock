...
saveState: {
      ...
      columns: [{
      ...
      },
       {
        ...
        template: function (row) {
          if (row.file ) {
            return `${Number(row.file.downloads.length)}`;
          } else {
            return '0';
          }
        },
    ...
    //   ,
       {
        ...
        template: function(row) {
          ...
          return '<span class="btn btn-bold btn-sm btn-font-sm ' + statuses[row.contributor_stage] + '">' + statuses[row.contributor_stage] + '</span>';
        }
      },
