                                ...
                                <div class="col-md-4 kt-margin-b-20-tablet-and-mobile">
                                    <div class="kt-form__group kt-form__group--inline">
                                        ...
                                        <div class="kt-form__control">
                                            <select name="role" class="form-control" id="kt_form_role">
                                                @foreach(trans('admin.roles') as $k=>$r)
...
@push('scripts')
    <script>
        ...
        var KTUserListDatatable = function () {
            var $roles =@json(trans('admin.roles'));
                ...
                datatable = $('#kt_member_list_datatable').KTDatatable({
                    ...
                    data: {
                        ...
                        source: {
                            read: {
                                method: 'get',
                                url: '{{ route('admin.members.datatable') }}',
                                params: {
                                    @if(request()->all())
              ...
              <div class="kt-user-card-v2__details">\
                ...
                <span class="kt-user-card-v2__desc">' + row.email + '</span>\
                    ...
                    },
                        {
                            ...
                            textAlign: 'center',
                            template: function (row, index, datatable) {
                                if (row.team && row.team.leader_id == row.id)
                                    return $roles[row.role] + '- مدير فريق (' + row.team.name + ')';
                        ...
                        },
                        {
                            ...
                            template: function (row, index, datatable) {
                                if (!row.team)
                                    return '-';
                                var $name = row.team.name;
                                if (row.team.leader_id == row.id)
                                    $name += ' - مدير';
                                else
                                    $name += ' - عضو';
                                return $name;
                            ...
                            {{--{--}}
                            {{--    field: 'published_images_count',--}}
                            {{--    title: '{{__('views.Published Images')}}',--}}
                            {{--    textAlign: 'center',--}}
                            {{--},--}}
                        {
                            field: "Actions",
