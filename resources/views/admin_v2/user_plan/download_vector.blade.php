            ...
            <div class="kt-portlet__body kt-portlet__body--fit">
                <div class="col-md-12">
                    <div class="kt-form kt-form--label-right kt-margin-t-20 kt-margin-b-10">
                        <form action="{{route('admin.vector.user_plans.items.export_downloads')}}" >
                ...
                datatable = $('#kt_user_plan_downloads_list_datatable').KTDatatable({
                    ...
                    columns: [
                        ...
                        },
                        {
                            ...
                            template: function(row, index, datatable) {
                                return (row.vector)?row.vector.user.name:'-'
                            },
                        },
                        {
                            field: 'ip',
...

                        {
                            ...
                            template: function (row, index, datatable) {
                                if (row.vector && row.vector.title)
                                    return '<a target="_blank" href="' + row.vector.post_link + '">' + row.vector.title + '</a>';
                        ...
                        },
                        {
                            ...
                            template: function (row, index, datatable) {
                                return row.purchase ? row.purchase.profit_ratio : '0';
                            },
                        },
                        {
                            field: 'profit_value',
                            ...
                            template: function (row, index, datatable) {
                                return row.purchase ? row.purchase.profit_value : '0';
                            },
                        },
                        {
                            field: 'unit_price',
                            ...
                            template: function (row, index, datatable) {
                                return row.purchase ? row.purchase.unit_price : ((row.subscription && row.plan) ? Math.round(row.subscription.amount / row.plan.downloads_count) : '0');
                            },
                        },
                        {
                            field: 'date',
