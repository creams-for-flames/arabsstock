            ...
            <div class="kt-portlet__body kt-portlet__body--fit">
                <div class="col-md-12">
                    <div class="kt-form kt-form--label-right kt-margin-t-20 kt-margin-b-10">
                        <form action="{{route('admin.user_plans.items.export_downloads')}}" method="get">
                                        ...
                                        <div class="col-md-2 kt-margin-b-20-tablet-and-mobile">
                                            <div class="form-group">
                                                ...
                                                <select name="created_by" id="created_by"
                                                        class="form-control select2_folder">
...
// columns definition
                    columns: [{
                        ...
                        template: function (row, index, datatable) {
                            return (row.image) ? row.image.user.name : '-'
                        },
                    },
                        {
                            field: 'imageshow',
...
// var image=row.image.thumbnail;
                                if (row.image && row.image.preview)
                                    var image_url = '{{url('')}}/' + row.image.preview;
                                if (row.image && row.image.preview)
                                    return '<img width="100"  src="{{ cdn('') }}/' + image_url + '" alt="image">';
                        ...
                        },
                        {
                            ...
                            template: function (row, index, datatable) {
                                return row.purchase ? row.purchase.plan_price : (row.subscription ? row.subscription.amount : '0');
                            },
                        },
                        {
                            field: 'unit_price',
                            ...
                            template: function (row, index, datatable) {
                                return ((row.subscription && row.plan) ? Math.round(row.subscription.amount / row.plan.downloads_count) : '0');
                            },
                        },
                        {
                            field: 'date',
                        ...
                        },
                        {
                            ...
                            template: function (row) {
                                var show_url = '{{ route('admin.downloads.show',0) }}'.replace('/0', '/' + row.id),
                                    delete_url = '{{ route('admin.user_plans.delete_download',0) }}'.replace('/0', '/' + row.id)
                                return '\
                                        <div class="dropdown">\
