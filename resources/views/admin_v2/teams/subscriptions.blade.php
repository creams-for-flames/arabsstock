                                    ...
                                    <div class="col-md-2 kt-margin-b-20-tablet-and-mobile">
                                        <div class="form-group">
                                            ...
                                            <select class="form-control select2-input" id="plan_id"
                                                    name="plan_id">
                                                <option></option>
                                                @foreach(\App\Models\Plan::where('status',1)->where('for_teams',1)->orderBy('id')->get() as $r)
                                                    <option
                ...
                datatable = $('#kt_user_plan_list_datatable').KTDatatable({
                    ...
                    columns: [{
                        ...
                        template: function (row, index, datatable) {
                            return row.plan?row.plan.description:'#'
                        },
                    },
                        {
                            field: 'user_id',
                        ...
                        },
                        {
                            ...
                            template: function (row, index, datatable) {
                                var show_url = '{{ route('admin.subscriptions.show',0) }}'.replace('/0', '/' + row.id),
                                    edit_url = '{{ route('admin.subscriptions.edit',0) }}'.replace('/0', '/' + row.id);
                                return '\
              <div class="dropdown">\
