...
@section('meta')
    <link rel="canonical" href="{{ route('similar.files',array_filter(['type'=>'photos','section'=>$section,'id'=>$file->id,'page'=>request('page')?:''])) }}"/>
            ...
            @if( $results->total() != 0 )
                <div id="imagesFlex" class="flex-images">
                    @foreach( $results as $item )
                        @include('images.item',['image'=>$item])
                    @endforeach
                </div>
                @if( $total != 0 )
                    <div class="row mt-4">
