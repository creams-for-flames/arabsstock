...
class UpdateStageEditVideo implements ShouldQueue
{
    ...
    {
        $ids = $this->ids;
        $data = Video::with(['category','tags'])->whereIn('id',$ids)
                    ->get();
        foreach ($data as $key => $item) {
            ...
            if(
                ...
                if ( $stage_edit !=  $last_stage_edit) {
                    ...
                    $item->save();
                    Log::channel('info')->info("UpdateStageEditVideo id : {$item->id} stage edit from :{$last_stage_edit} to:  {$item->stage_edit}");
                }
