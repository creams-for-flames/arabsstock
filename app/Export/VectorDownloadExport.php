...
class VectorDownloadExport implements WithMapping, WithHeadings, FromCollection, ShouldAutoSize
{
    protected $Objects;
    ...
    {
        $purchase = Purchase::where('purchaseable_type', Vector::class)->where('purchaseable_id', $object->vector_id)->where('user_id', $object->user_id)->first();
        return [
            $object->id,
            $object->vector_id,
            $object->client ? $object->client->name : '-',
            $object->ip,
            $object->subscription && $object->subscription->plan ? $object->subscription->plan->title_en : '-',
            ...
            $object->subscription && $object->subscription->plan ? round(($object->subscription->amount / $object->plan->downloads_count), 2) : '0',
    ...
    public function headings(): array
    {
        ...
        array_push($heading, __('global.vector'));
