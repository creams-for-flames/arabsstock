...
class CategoryAdminController extends Controller
{
    ...
    {
        $index_url = route('admin.categories_admin.datatable');
        $edit_url = route('admin.categories_admin.edit', 0);
        $destroy_url = route('admin.categories_admin.destroy', 0);

        $object = new CategoryAdmin();
        ...
        $html_new_path = route('admin.categories_admin.create');
        $subheader_actions = [
            'delete' => [
                'type' => 'button',
                'text' => __('views.Delete All'),
                'url' => route('admin.categories_admin.destroy', 0),
                'method' => 'delete',
                'confirm' => __(
    ...
    {
        $data = process_datatable_query(CategoryAdmin::query(), function (
            $query,
            $search
        ) {
            return $query
    ...
    {
        $index_url = route('admin.categories_admin.index');
        $store_url = route('admin.categories_admin.store');
    ...
    public function store(Request $request)
    {
        ...
        $sql = new CategoryAdmin();
        $sql->name = trim($request->name);
        $sql->slug = preg_replace('/\s+/', '', $request->name);
        $sql->save();

        \Session::flash('success', trans('admin.success_add_category'));
        return redirect()->route('admin.categories_admin.index');
    }

    public function edit($id)
    {
        $categories = CategoryAdmin::find($id);

        $index_url = route('admin.categories_admin.index');
        $update_url = route('admin.categories_admin.update', $id);
    ...
    public function index_video()
    {
        ...
        $index_url = route('admin.videos.categories_admin.datatable');
        $edit_url = route('admin.videos.categories_admin.edit', 0);
    ...
    public function datatable_video(Request $request)
    {
        ...
        $data = process_datatable_query(CategoryVideoAdmin::query(), function (
            $query,
            $search
        ) {
            return $query
    ...
    public function edit_video($id)
    {
        ...
        $categories = CategoryVideoAdmin::find($id);
