...
class VectorReviewsController extends Controller
{
    /**
    ...
    public function index(Request $request)
    {
        if (!$request->datatable) {
            $index_url = route('admin.vector-reviews.index');

            $html_breadcrumbs = [
                'title' => __('Reviewed Vectors'),
                'subtitle' => __('views.Index'),
                'datatable' => true,
            ];

            return view(
                'admin_v2.vector-reviews.index',
                compact(
                    'html_breadcrumbs',
                    'index_url'
                )
