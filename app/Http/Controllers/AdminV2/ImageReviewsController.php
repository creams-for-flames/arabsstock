...
class ImageReviewsController extends Controller
{
    /**
    ...
    public function index(Request $request)
    {
        if (!$request->datatable) {
            $index_url = route('admin.image-reviews.index');

            $html_breadcrumbs = [
                'title' => __('views.Reviewed Images'),
                'subtitle' => __('views.Index'),
                'datatable' => true,
            ];

            return view(
                'admin_v2.image-reviews.index',
                compact(
                    'html_breadcrumbs',
                    'index_url'
                )
