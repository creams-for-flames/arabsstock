...
class CollectionController extends Controller
{
    ...
    public function addImageCollection(Request $request, $id, $id2)
    {
        if (Auth::check()) {
            if ($request->ajax()) {
    
                $collectionsImage = CollectionImage::where([
                    'collection_id' => $id,
                    'image_id' => $id2
                ])->first();
                ...
                if (isset($collectionsImage)) {
                    ...
                    $saveCollectionsImage = new CollectionImage;
                    $saveCollectionsImage->collection_id = $id;
                    $saveCollectionsImage->image_id = $id2;
    ...
    public function edit(Request $request)
    {
        ...
        $sql = ImageCollection::find($request->id);
    ...
    public function destroy($id)
    {
        ...
        $collection = ImageCollection::findOrFail($id);
    ...
    public function deleteImageCollection($collectionID, $imageID)
    {
        $collection = ImageCollection::findOrFail($collectionID);
        $collectionsImages = CollectionImage::where('collection_id', $collection->id)
            ->where('image_id', $imageID)->first();
        $collectionsImages->delete();
        return redirect()->back();
    }
}
