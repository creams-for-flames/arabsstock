...
use App\Models\ImageCommentLike;
...
class CommentsController extends Controller
{
    ...
    public function __construct(AdminImageSettings $settings, Request $request)
    {
    ...
    public function store(Request $request)
    {
        ...
        $image = Image::where('id', $request->image_id)->where('status', 'active')->first();
...
<ul class="list-inline collapse text-right" id="collapse' . $idComment . '">
    ...
    public function like(Request $request)
    {
        ...
        $comment_like = ImageCommentLike::where('user_id', '=', Auth::user()->id)
        ...
        if (isset($comment_like->id)) {
            ...
            if ($comment_like->status == '1') {
                ...
                $comment_count = ImageCommentLike::where('comment_id', '=', $id)->where('status', '1')->count();
            ...
            if (Auth::user()->id != $comment->user_id) {
                Notifications::send($comment->user_id, Auth::user()->id, '4', $comment->image_id);
            }
    ...
    public function getLikes(Request $request)
    {
        ...
        $data = ImageCommentLike::where('comment_id', $comment_id)->where('status', '1')->get();
        ...
        foreach ($data as $key) {
            $_array[] = '<li><a href="' . url($key->user()->username) . '" class="showTooltip" data-toggle="tooltip" data-placement="left" title="' . $key->user()->username . '">
					<img src="' . asset('avatar') . '/' . $key->user()->avatar . '" class="img-circle" width="25">
					</a></li>';
        }
        return $_array;
