...
trait userTraits {
	...
	if( isset( $collections ) ){
		foreach($collections as $collection){
			...
			// Collection
		$collectionsImages = CollectionImage::where('image_id', '=', $collection->id)->get();
   ...
   $collection->delete();
	...
	// Likes
	$likes = ImageLike::where('user_id', '=', $id)->get();
	if( isset( $likes ) ){
	...
	// ImageDownload
	$downloads = ImageDownload::where('user_id', '=', $id)->get();
	if( isset( $downloads ) ){
	...
	if(isset( $images )){
		foreach($images as $image){
			...
			$stocks = Stock::where('image_id', '=', $image->id)->get();

			foreach($stocks as $stock){

				$stock_path = public_path('uploads/'.$stock->type.'/'.$stock->name);
	...
	//<<<-- Delete Avatar -->>>/
	$fileAvatar    = public_path('avatar/'.$user->avatar);
	...
	//<<<-- Delete Cover -->>>/
	$fileCover  = public_path('cover/'.$user->cover);
