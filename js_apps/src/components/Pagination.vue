<template>
    <div class="kt-pagination kt-pagination--brand kt-margin-t-10">
        <template v-if="hasPages()">
            <ul class="kt-pagination__links">
                <li v-if="onFirstPage()" class="kt-pagination__link--first" :class="'kt-pagination__link--disabled'">
                    <a href="javascript:;" rel="prev"><i :class="'fa fa-angle-double-' + right"></i></a>
                </li>
                <li v-else class="kt-pagination__link--first">
                    <a @click.prevent="changePageNumber(1)" :href="previousPageUrl()" rel="prev"><i :class="'fa fa-angle-double-' + right"></i></a>
                </li>
                <li v-if="onFirstPage()" class="kt-pagination__link--next kt-pagination__link--disabled">
                    <a href="javascript:;" rel="prev"><i :class="'fa fa-angle-' + right"></i></a>
                </li>
                <li v-else class="kt-pagination__link--next">
                    <a @click.prevent="changePageNumber(params.page - 1)" :href="previousPageUrl()" rel="prev"><i :class="'fa fa-angle-' + right"></i></a>
                </li>

                <li v-for="item in prevPageCount" :key="`prev${item}`">
                    <a @click.prevent="changePageNumber(params.page - 1 - (prevPageCount - item))" :href="url(params.page - 1 - (prevPageCount - item))">{{ params.page - 1 - (prevPageCount - item) }}</a>
                </li>

                <li v-for="item in nextPageCount" :key="`next${item}`" :class="params.page - 1 + item === params.page ? 'kt-pagination__link--active' :((params.page - 1) + item ) == params.page?'kt-pagination__link--active':''">
                    <a v-if="params.page - 1 + item === params.page" href="javascript:;">{{ params.page - 1 + item }}</a>
                    <a v-else @click.prevent="changePageNumber(params.page - 1 + item)" :href="url(params.page - 1 + item)">{{ params.page - 1 + item }}</a>
                </li>

                <li v-if="hasMorePages()" class="kt-pagination__link--prev">
                    <a @click.prevent="changePageNumber(params.page + 1)" :href="nextPageUrl()" rel="next"><i :class="'fa fa-angle-' + left"></i></a>
                </li>
                ...
                <li v-if="hasMorePages()" class="kt-pagination__link--last">
                    <a @click.prevent="changePageNumber(lastPage())" :href="nextPageUrl()" rel="next"><i :class="'fa fa-angle-double-' + left"></i></a>
                </li>
        ...
        <div class="kt-pagination__toolbar" id="page">
            <div class="form-group text-center">
                ...
                @keyup.enter="changePageFromTextInput"
                >
...
export default {
    ...
    methods: {
        ...
        changePageFromTextInput(){
            this.params.page = this.page??this.params.page;
