...
$('#buttonSearch, #btnSearch, #_buttonSearch').click(function (e) {
  ...
  var search = $('#btnItems').val().replace(/\#+/gi, '%23');
...
$('#btnSearch_2').click(function (e) {
  var search = $('#btnItems_2').val().replace(/\#+/gi, '%23');
  var target = URL_BASE + '/search/' + search;
  if (trim(search).length < 2 || trim(search).length == 0 || trim(search).length > 100) {
    return false;
  } else {
...
$('#btnItems_2').keydown(function (event) {
  var keypressed = event.keyCode || event.which;
  if (keypressed == 13) {
    var search = $(this).val().replace(/\#+/gi, '%23');
    var target = URL_BASE + '/search/' + search;
...
$('#buttonSearch2, #btnSearch2, #_buttonSearch2').click(function (e) {


  var search = $('#btnItems2').val().replace(/\#+/gi, '%23');


  var target = URL_BASE + '/video/search/' + search;
  alert(target);
  if (trim(search).length < 2 || trim(search).length == 0 || trim(search).length > 100) {
    return false;
  } else {
...
$('#btnItems2').keydown(function (event) {
...
$('#btnItems_22').keydown(function (event) {
  var keypressed = event.keyCode || event.which;
  if (keypressed == 13) {
    var search = $(this).val().replace(/\#+/gi, '%23');
    var target = URL_BASE + '/video/search/' + search;
...
//<---------- * Avatar * ------------>>
  ...
  $(".likeVideoButton").on('click', function (e) {
    ...
    $.ajax({
      ...
      type: "POST",
      url: URL_BASE + "/video/ajax/like",
      data: data,
      success: function (result) {

        if (result == '') {
          window.location.reload();
          element.removeClass('likeVideoButton');
          element.removeClass('active');
        } else {
          //element.parents('.actions').find('.like_count').html( result );
          element.find('i').removeClass('icon-spinner2 fa-spin');
...
/*= Add collection  =*/
$("#addVideoCollection").on('click', function (e) {
  var element = $(this);
  ...
  $.ajax({
    ...
    type: "POST",
    url: URL_BASE + "/video/collection/store",
    dataType: 'json',
    data: $("#addVideoCollectionForm").serialize(),
    success: function (result) {
      ...
      if (result.success == true) {
        ...
        $('#collectionsVideo .no-collections').remove();
        $("#collectionsVideo #titleCollection").val('');
...
//<--------- Edit Collection
$("#editVideoCollection").on('click', function (e) {
  var element = $(this);
...
function addVideoCollection() {
  ...
  $(".addVideoCollection").click(function () {
    ...
    $.ajax({
      ...
      type: "GET",
      url: URL_BASE + '/video/collection/' + collectionID + '/i/' + imageID,
      dataType: 'json',
      data: null,
      success: function (response) {
        $('#collectionsVideo').modal('hide');
